...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conrelid = 'public.attribute_values'::regclass
      AND contype = 'p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conrelid = 'public.attributes'::regclass
      AND conname = 'attributes_name_key'
  ) AND NOT EXISTS (
    SELECT 1
    FROM pg_class c
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE c.relname = 'attributes_name_key'
...
BEGIN
  IF NOT EXISTS (
    ...
    FROM pg_class c
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE c.relname = 'companies_slug_key'
      AND n.nspname = 'public'
  ) THEN
    ALTER TABLE ONLY public.companies
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conrelid = 'public.price_tiers'::regclass
      AND contype = 'p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conrelid = 'public.product_supplies'::regclass
      AND contype = 'p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    FROM pg_class c
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE c.relname = 'products_sku_key'
      AND n.nspname = 'public'
  ) THEN
    ALTER TABLE ONLY public.products
...
BEGIN
  IF NOT EXISTS (
    SELECT 1
    FROM pg_constraint
    WHERE conrelid = 'public.supplies'::regclass
      AND contype = 'p'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    FROM pg_class c
    JOIN pg_namespace n ON n.oid = c.relnamespace
    WHERE c.relname = 'user_roles_user_id_role_key'
      AND n.nspname = 'public'
  ) THEN
    ALTER TABLE ONLY public.user_roles
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public'
      AND tablename = 'companies'
      AND policyname = 'Users without company can create their first company'
  ) THEN
