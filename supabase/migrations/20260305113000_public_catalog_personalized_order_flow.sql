-- Public catalog checkout: enforce personalized reference upload and pending-customer-info flow.

drop policy if exists "Public catalog order art temp upload" on storage.objects;
create policy "Public catalog order art temp upload"
  on storage.objects
  for insert
  ...
  with check (
    ...
    and (storage.foldername(name))[1] = 'public-catalog'
    and (storage.foldername(name))[2] ~* '^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$'
  );

drop policy if exists "Public catalog order art temp read" on storage.objects;
create policy "Public catalog order art temp read"
  on storage.objects
  for select
  to anon, authenticated
  using (
...
begin
  ...
  loop
    ...
    if v_item_is_personalized then
      ...
      if v_reference_file_path is null then
        raise exception 'Reference file is required for personalized product';
      end if;
