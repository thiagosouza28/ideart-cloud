...
const getCustomerStorageKey = () => {
  try {
    ...
    return projectRef ? `sb-${projectRef}-customer-auth-token` : undefined;
  } catch {
    return undefined;
  }
};
...
export const customerSupabase = createClient<Database>(SUPABASE_URL, SUPABASE_CLIENT_KEY, {
  auth: {
    storage: localStorage,
    storageKey: getCustomerStorageKey(),
    persistSession: true,
